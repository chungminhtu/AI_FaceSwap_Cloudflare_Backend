...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // Handle external resource proxy for CORS bypass (e.g., Lottie animations)
    // GET /proxy-resource?url=<encoded-url>
    if (path === '/proxy-resource' && request.method === 'GET') {
      try {
        const url = new URL(request.url);
        const targetUrl = url.searchParams.get('url');

        if (!targetUrl) {
          return errorResponse('url parameter is required', 400, undefined, request, env);
        }
        ...
        // Only allow specific domains for security (whitelist approach)
        const allowedDomains = [
          'resources.d.shotpix.app',
          'cdn.jsdelivr.net',
          'unpkg.com'
        ];
        ...
        if (!isAllowedDomain) {
          return errorResponse('Domain not allowed', 403, undefined, request, env);
        }
        ...
        const response = await fetchWithTimeout(targetUrl, {
          method: 'GET',
          headers: {
            'User-Agent': 'Mozilla/5.0 (compatible; FaceSwap Proxy)',
            'Accept': '*/*'
          }
        }, TIMEOUT_CONFIG.IMAGE_FETCH);

        if (!response.ok) {
          return errorResponse(`Failed to fetch resource: ${response.status}`, response.status, undefined, request, env);
        }
        ...
        logCriticalError('/proxy-resource', error, request, env);
        const errorMsg = error instanceof Error ? error.message : String(error);
        const debugEnabled = isDebugEnabled(env);
        return errorResponse('Failed to proxy resource', 500, debugEnabled ? { error: errorMsg.substring(0, 200) } : undefined, request, env);
      }
    }
