  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (path === '/removeBackground' && request.method === 'POST') {
      try {
        ...
        const defaultMergePrompt = `You are a professional photo compositor. Your task is to place the person from the first image (which has a transparent background) into the second image (the landscape scene), making them look like they naturally belong in that scene.
